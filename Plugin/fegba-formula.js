...
(function() {
	...
	AbilityCalculator.getAvoid = function(unit) {
		...
		avoid = (this.getAgility(unit, ItemControl.getEquippedWeapon(unit))) + RealBonus.getLuk(unit);
	...
	DamageCalculator.calculateAttackPower = function(active, passive, weapon, isCritical, totalStatus, trueHitValue) {
		...
		if (this.isEffective(active, passive, weapon, isCritical, trueHitValue)) {
			pow += weapon.getPow() * (this.getEffectiveFactor()  ); //Example: If effective factor is 3, then you need to add weapon might * 2 to the total power
